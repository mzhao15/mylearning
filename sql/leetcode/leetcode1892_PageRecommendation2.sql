...
SELECT
    f.user1_id, l1.page_id, COUNT(f.user2_id) AS friends_likes
FROM (
...
JOIN Likes l1
ON
    f.user2_id = l1.user_id
WHERE NOT EXISTS (
    SELECT * FROM Likes l2
    WHERE f.user1_id = l2.user_id AND l1.page_id = l1.page_id
)
GROUP BY
    f.user1_id, l1.page_id
