...
def consume_topic(topic, group, output_dir, frequency):
    ...
    consumer = KafkaConsumer(topic, group_id=group, bootstrap_servers=['localhost:9092'],fetch_max_bytes=1310720000)
    ...
    while True:
        messages = consumer.get_messages(count=1000, block=False) #get 1000 messages at a time, non blocking
        # messages = consumer.poll(max_records = 1000)
        if not messages:
        ...
        consumer.commit() # inform zookeeper of position in the kafka queue
    # 
